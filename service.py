...
producer = AIOKafkaProducer(bootstrap_servers="localhost:9092", loop=event_loop)
...
class MailService:
    ...
    def send_welcome_email(self, subject, text, to):
        self.mail_client.send_message(subject=subject, text=text, to=to)

    async def send_fail_email(self, dic: dict) -> None:
        await producer.start()
        try:
            await producer.send("callback_email_topic", value=json.dumps(dic).encode())
        finally:
            await producer.stop()
